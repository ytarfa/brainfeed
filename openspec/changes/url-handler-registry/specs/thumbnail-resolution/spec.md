...
**Reason**: OG image resolution is now handled by the `OgFetcher` base layer in the `UrlHandlerRegistry`. The registry always fetches OG metadata and maps `og:image` to `ResolvedBookmark.thumbnailUrl` as the default. There is no separate "generic OG image resolution" step — it is the base behavior of the registry itself.
**Migration**: Use `UrlHandlerRegistry.resolve(url)` which returns `ResolvedBookmark.thumbnailUrl` populated from `og:image` when no handler overrides it.

### Requirement: Thumbnail resolution function architecture
**Reason**: The `resolveThumbnail(url, sourceType)` function and `ThumbnailService` class are replaced by the `UrlHandlerRegistry`. Thumbnail resolution is no longer a standalone concern — it is a field in `ResolvedBookmark` produced by the unified handler system.
**Migration**: Replace `resolveThumbnail(url, sourceType)` calls with `registry.resolve(url)` and use the `.thumbnailUrl` field from the returned `ResolvedBookmark`.
