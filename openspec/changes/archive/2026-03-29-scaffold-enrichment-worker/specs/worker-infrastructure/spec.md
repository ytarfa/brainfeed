...
The `worker-core` package SHALL validate required environment variables at initialization and export a typed configuration object.

#### Scenario: Required environment variables
- **WHEN** the worker-core configuration is initialized
- **THEN** it SHALL require `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`, and accept optional `REDIS_HOST`, `REDIS_PORT`, and `REDIS_PASSWORD` with sensible defaults
