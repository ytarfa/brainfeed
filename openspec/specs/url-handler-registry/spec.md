...
The system SHALL evaluate handlers in registration order and use the first handler where `matches()` returns true. Subsequent handlers SHALL NOT be evaluated after a match is found. The registration order SHALL be: YouTube, GitHub, Article.
...
- **WHEN** a URL has `og:type` of `"article"` and does not match YouTube or GitHub handlers
